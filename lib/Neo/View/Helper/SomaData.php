...
class Neo_View_Helper_SomaData {
	...
	public function SomaData($data_ini, $soma, $periodo){		
		...
		* $data_ini	= É a data cujo a Funçao vai somar

		* $soma		= É a quantidade que vai ser somada (Depende do Periodo para somar dia, mes ou ano)

		* $periodo	= Serve para informar o que vai ser somado (dia, mes, ano) se Periodo for = soma ele pega

		* a variavel $data_inicial (é passado aqui a data inicial e a final uninco com -) e ve quantos dias de

		* diferença entre o Período.

		*

		* Ex1: SomaData('2010-01-01'.'-'.'2010-01-05', '', 'soma');  ->  Retorna a diferença entre as datas (4 dias)
		...
		*/
		...
		if($periodo == 'soma')	{
			...
			$dia1	= $data[2]; $dia2	= $data[5];

			$mes1	= $data[1]; $mes2	= $data[4];

			$ano1	= $data[0];	$ano2	= $data[3];				
